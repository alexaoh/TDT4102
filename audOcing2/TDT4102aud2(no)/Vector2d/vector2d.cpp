...
//Skjønner ikke helt om det er meningen å implementere i en header-fil? Tror ikke det er meningen, da alle includes.etc er her.
class Vector2d{
public: 
  double x; 
  double y;
...
void trackStats(const vector<Vector2d>& track){
  double length{0};
  double maxSpeed{0};
  Vector2d endVec = vectorSum(track);
  double speed;
  for (unsigned int i = 0; i<track.size(); ++i){
    length += track[i].length();
    speed = (track[i].length()*1000)/10;
    if (speed > maxSpeed) maxSpeed = speed;
  }
  cout << setprecision(2) << fixed << "Length: " << length << ", max-speed: " 
       << setprecision(0) << fixed << maxSpeed << " m/min, ended at " << endVec;
...
Vector2d outsideTrack(Vector2d& line){
  if (line.x > 10.0 || line.x<0.0|| line.y > 10.0 || line.y < 0.0){
    line.x = 0.0;
    line.y = 0.0;
  }
  return line;
...
void cleanTrack(vector<Vector2d>& track){
  transform(track.begin(), track.end(), track.begin(), outsideTrack);
}
