...
// BEGIN: S2
  ...
  Reading SensorsRandom::get_reading(unsigned long timestep,string name){
    Reading reads{0, 0};
    if (timestep % 3 == 0){ //Hvert 3. timestep for et random tall for inflow og outflow, litt større svingninger enn foreslått i oppgaveteksten. 
      reads.inflow = rand() % 100;
      reads.outflow = rand() % 100;
    }
    return reads;
...

{
  ...
  ifstream ist{fname};
  if (!ist) throw runtime_error("Cannot open file "+fname);
  //Dette er en grei sjekk å ha uansett. Enkel å implementere er den også. 

  for (string line; getline(ist, line); ){
    int timestep, inflow, outflow;
    char comma1, comma2; 
    string dam;
    istringstream iss{line};
    iss >> timestep >> comma1;
    getline(iss, dam, ',');
    iss >> inflow >> comma2 >> outflow;
    //antar rett format, som skrevet i oppgaveteksten. 
