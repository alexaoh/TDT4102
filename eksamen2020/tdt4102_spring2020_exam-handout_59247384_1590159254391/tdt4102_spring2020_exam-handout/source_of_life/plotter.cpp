...
{
  // BEGIN: P1
  Color::Color_type col = colors[cur_color];
  cur_color++;
  return col;
...
{
  // BEGIN: P2
  srand(static_cast<unsigned int>(time(nullptr))); //F.eks .
  int n = colors.size();
  for (int i = 0; i < n-1; ++i){
    int j = rand() % (n-i) + i; 
    auto temp = colors[i];
...
Color::Color_type Plotter::get_color(const string dam)
{
  ...
  try{
      Color::Color_type col = dam_colors.at(dam); //If the dam-key does not exist, it throws an exception (out of range). 
      return col;
  } catch (std::out_of_range){ 
      Color::Color_type col = colors.next_color();
      dam_colors.insert({dam, col}); //Implicit conversion to pair. 
      return col;
  } //Antar at den skal returnere fargen uansett om den finnes eller ikke til slutt. 
...
Point Plotter::get_last_point(const string dam, Point current_point)
{
  ...
  try{ //.at throws an exception if the key is not found. 
    Point lastPoint = last_points.at(dam);
    last_points.at(dam) = current_point; 
    return lastPoint;
  } catch (std::out_of_range){  
    last_points.insert({dam, current_point}); //Implicit conversion to pair. 
    return {0,0}; //Implicit conversion to Point. 
  }
                         ...
                         Dam::WaterAlarm alarm)
{
  ...
  if(alarm != Dam::WaterAlarm::NoAlarm){ //An alarm has been set off. 
    Point startPoint{calc_x(timestep), y_len + 40};
    Point endPoint{calc_x(timestep), y_len + 60};
    Line* line = new Line{startPoint, endPoint};
    line->set_color(get_color(dam));
    shapes.push_back(line); //Legger den til slik at den ryddes opp når plotter destrueres. 
    this->attach(*line);
    redraw();
  }
...
{
  // BEGIN: P6
  Point startPoint{calc_x(0), calc_y(limit)};
  Point endPoint{calc_x(timesteps-1), calc_y(limit)};//F.eks, siden calc_x krever at x < timesteps.
  Line* horizontal = new Line{startPoint, endPoint};
  horizontal->set_style(Line_style::dash);
  horizontal->set_color(get_color(dam));
