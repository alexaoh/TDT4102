...
Dam::WaterAlarm Dam::get_alarm() const
{
  ...
  if (water_level > water_level_max){
    return WaterAlarm::WaterLevelHigh;
  } else if (water_level < water_level_min){
    return WaterAlarm::WaterLevelLow;
  } else {
    return WaterAlarm::NoAlarm;
  }
...
void Dam::restore_water_level()
{
  ...
  int water_delta = water_level_delta();
  if (water_delta != 0){ //Means that the water_level differs from ideal_level (either over or under).
  //Ser i ettertid at denne sjekken gjøres i do_timestep(), men rekker ikke endre på det. Ikke veldig skadelig å ha den der uansett. 
    if (water_delta < inflow_capacity || water_delta < outflow_capacity){
      water_level = ideal_level;
    } else {
      water_level = water_level + (inflow_value - outflow_value); //Tolker det som at denne kjøres per timestep (i do_timestep()). 
      //Skulle gjerne ganget med timestep her, siden det er l/min, men trengs ikke da jeg antar at timestep = 1. 
      //I tillegg antar jeg at jeg skal bruke inflow_value og outflow_value og ikke capacity til hhv inflow og outflow. 
    }
  }
