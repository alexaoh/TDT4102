...
void DamSim<SensorProvider>::write_data(Dam &v, int cur_timestep)
{
  ...
  //output_file og output_file_name settes i konstruktøren og det sjekkes om fila kan åpnes der. Gjør det dermed ikke her. 
  string damName = v.get_name();
  int waterLevel = v.get_water_level();
  //Antar at "krokodille-tegnene" ikke skal med! 
  output_file << damName << ',' << cur_timestep << ',' << waterLevel << '\n';
